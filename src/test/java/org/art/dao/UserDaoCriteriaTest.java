...
public class UserDaoCriteriaTest {
    ...
    private final UserDaoCriteria userDaoCriteria = UserDaoCriteria.getInstance();
    ...
    void findAll() {
        ...
        List<User> results = userDaoCriteria.findAll(session);
        ...
        assertThat(fullNames).containsExactlyInAnyOrder("Bill Gates", "Steve Jobs",
                "Sergey Brin", "Tim Cook", "Diane Greene");
    ...
    void findAllByFirstName() {
        ...
        List<User> results = userDaoCriteria.findAllByFirstName(session, "Bill");
    ...
    void findLimitedUsersOrderedByBirthday() {
        ...
        int limit = 3;
        List<User> results = userDaoCriteria.findLimitedUsersOrderedByBirthday(session, limit);
    ...
    void findAllByCompanyName() {
        ...
        List<User> results = userDaoCriteria.findAllByCompanyName(session, "Google");
    ...
    void findAllPaymentsByCompanyName() {
        ...
        List<Payment> applePayments = userDaoCriteria.findAllPaymentsByCompanyName(session, "Apple");
    ...
    void findAveragePaymentAmountByFirstAndLastNames() {
        ...
        Double averagePaymentAmount = userDaoCriteria.findAveragePaymentAmountByFirstAndLastNames(session, "Bill", "Gates");
    ...
    void findCompanyNamesWithAvgUserPaymentsOrderedByCompanyName() {
        ...
        List<Object[]> results = userDaoCriteria.findCompanyNamesWithAvgUserPaymentsOrderedByCompanyName(session);
    ...
    @Disabled
    @Test
    void findCompanyNamesWithAvgUserPaymentsOrderedByCompanyNameDto() {
        @Cleanup Session session = sessionFactory.openSession();
        session.beginTransaction();

        List<CompanyDto> results = userDaoCriteria
                .findCompanyNamesWithAvgUserPaymentsOrderedByCompanyNameDTO(session);
        assertThat(results).hasSize(3);
    ...
    void isItPossible() {
        ...
        List<Object[]> results = userDaoCriteria.isItPossible(session);
        assertThat(results).hasSize(2);
    ...
    @Disabled
    @Test
    void isItPossibleTuple() {
        @Cleanup Session session = sessionFactory.openSession();
        session.beginTransaction();

        List<Tuple> results = userDaoCriteria.isItPossibleTuple(session);
        assertThat(results).hasSize(2);
